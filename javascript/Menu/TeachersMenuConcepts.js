  ...
  try {
    ...
    for (var i = 0; i < 2; i++) {
      for (var j = 0; j < couples[i]?.length; j++) {
        $("#tableConcepts").append(
          "<tr>" +
            "<td contenteditable='false'>" +
            couples[i][j].id +
            "</td>" +
            "<td contenteditable='true'>" +
            getMatterById(couples[i][j].idMatter) +
            "</td>" +
            "<td contenteditable='true'>" +
            couples[i][j].concept +
            "</td>" +
            "<td contenteditable='true'>" +
            couples[i][j].definition +
            "</td>" +
            "" +
            "<td>" +
            "<button class='btn btn-primary' type='button' onclick='editCouple(\"#tableConcepts\")'>Actualizar</button> " +
            "</td>" +
            "<td>" +
            "<button class='btn btn-primary' type='button' onclick='deleteTableElement(\"#tableConcepts\")'>Eliminar</button> " +
            "</td>" +
            "</tr>"
        );
      }
